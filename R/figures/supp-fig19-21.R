...
for (percentile_type in percentile_types) {
    ...
    ggsave(paste0("fig/Supp_Fig19-21//luecken-normalization-boxplot-number-", percentile_type, ".pdf"), p1, width = 9, height = 15, units = "cm", useDingbats = FALSE)
    
    delta = dat0 %>% 
        group_by(da_method, label, percentile) %>% 
    ...
    p2
    ggsave(paste0("fig/Supp_Fig19-21/luecken-normalization-boxplot-number-", percentile_type, "-d.pdf"), p2, width = 9, height = 13, units = "cm", useDingbats = FALSE)
}
