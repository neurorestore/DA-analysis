...
# load data
dat = readRDS("data/summaries/concordance_res/multiome/all_aucc_logFC_absolute_filter.rds") %>% 
    type_convert()
...
p1
ggsave("fig/Exp_Fig2/multiome-concordance-absolute-logFC-filter-overview.pdf", p1,
...
p2
ggsave("fig/Exp_Fig2/multiome-concordance-absolute-logFC-filter-enrichment.pdf", p2,
       width = 16, height = 6, units = "cm", useDingbats = FALSE)
...
p3
ggsave("fig/Exp_Fig2/multiome-concordance-absolute-logFC-filter-d.pdf", p3,
       width = 4, height = 4, units = "cm", useDingbats = FALSE)

p4 = wrap_plots(p1,p2,ncol=1)
ggsave("fig/Exp_Fig2/top-row.pdf", p4,
