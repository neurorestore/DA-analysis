...
for (percentile_type in levels(dat$percentile_type)) {
    ...
    p1
    ggsave(paste0("fig/Supp_Fig15/snapatac-boxplot-number-", percentile_type, ".pdf"), p1,
    ...
    prop = dat0 %>% 
        mutate(decile = gsub("^.*_", "", percentile) %>% as.integer() %>% factor()) %>% 
        group_by(method, seed, binarization) %>% 
    ...
    p2 = prop %>% 
        ...
        theme(aspect.ratio = 1,
              legend.position = 'none')
    # p2
    ggsave(paste0("fig/Supp_Fig15/snapatac-boxplot-proportion-", percentile_type, ".pdf"), p2,
           width = 14, height = 7, units = "cm", useDingbats = FALSE)
    out_plots[[length(out_plots)+1]] = p2
}

full = wrap_plots(out_plots, ncol=2)
ggsave(paste0("fig/Supp_Fig15/full.pdf"), full,
