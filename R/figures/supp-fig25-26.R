...
dat = readRDS("data/summaries/false_discoveries/false_discoveries_simulated.rds") %>% 
    ...
    filter(n_cells == 1000, n_reps == 6, de_loc == 1, de_prob == 0.5)
...
percentile_types = c('mean', 'percentage_open')
...
for (percentile_type in percentile_types) {
    ...
    normalization_methods = normalization_methods[normalization_methods!='smooth GC-full quantile']
    ...
    ggsave(paste0("fig/Supp_Fig25-26/splatter-normalization-boxplot-number-", percentile_type, ".pdf"), p1, width = 9, height = 13, units = "cm", useDingbats = FALSE)
    
    delta = dat0 %>% 
        group_by(da_method, simulation_iter, percentile) %>% 
