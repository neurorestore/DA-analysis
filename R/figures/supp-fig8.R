...
dat %<>% filter(n_cells == 1000, n_reps == 6, de_loc == 1, de_prob==0.5)

# simulation parameters
table(dat$n_cells)
table(dat$n_reps)
table(dat$de_loc, dat$de_prob)
...
for (percentile_type in levels(dat$percentile_type)) {
    ...
    avg = dat0 %>% 
        group_by_at(vars(-simulation_iter, -number_of_regions, -number_of_da_regions,
    ...
    saveRDS(summary, paste0('data/summaries/meta_summaries/splatter_false_discoveries_', percentile_type, '_bias_summary.rds'))
    ...
    # p1
    ggsave(paste0("fig/Supp_Fig8/splatter-boxplot-number-", percentile_type, ".pdf"), p1,
    ...
    prop = dat0 %>% 
        group_by(method, simulation_iter) %>% 
    ...
    # p2
    ggsave(paste0("fig/Supp_Fig8/splatter-boxplot-proportion-", percentile_type, ".pdf"), p2,
           width = 17, height = 3, units = "cm", useDingbats = FALSE)
...
ggsave(paste0("fig/Supp_Fig8/full.pdf"), full,
